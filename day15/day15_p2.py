...
class WarehouseItem():
    ...
    def gps(self):
        if self.type == "[": return self.x + 100 * self.y
        else: return 1
...
def plotMap(width, height, walls, boxes, robot, move=None):
    ...
    for w in walls: map[w.y][w.x] = w.type
    for b in boxes: map[b.y][b.x] = b.type
    map[robot.y][robot.x] = robot.type
...
for y, line in enumerate(map):
    for x, item in enumerate(line):
        if item == "#":
            walls.append(WarehouseItem(item, x*2, y))
            walls.append(WarehouseItem(item, x*2+1, y))
        elif item == "O":
            boxes.append(WarehouseItem("[", x*2, y))
            boxes.append(WarehouseItem("]", x*2+1, y))
        elif item == "@":
            robot = WarehouseItem(item, x, y)
